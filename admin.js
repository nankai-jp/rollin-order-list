    ...
    try {
        ...
        items.forEach((item, index) => {
            ...
            tr.innerHTML = `
                ...
                <td style="border: 1px solid #000; padding: 6px 4px; max-width: 300px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${item.product_name}</td>
    ...
    try {
        ...
        items.forEach((item, index) => {
            ...
            tr.innerHTML = `
                ...
                <td style="max-width: 400px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${item.product_name}">${item.product_name}</td>
    ...
    filtered.forEach(prod => {
        ...
        if (images.length > 0) {
            ...
            images.forEach(imgUrl => {
                ...
                imgHtml += `<img src="${imgUrl}" alt="${safeAlt}" class="col-thumb-img" style="width: 40px; height: 40px; object-fit: cover; border-radius: 6px; cursor: pointer;" onclick="window.open('${safeUrl}', '_blank')">`;
            });
            imgHtml += `</div>`;
        } else {
            imgHtml = `<span style="color: var(--text-secondary); opacity: 0.4;">-</span>`;
